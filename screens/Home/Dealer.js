...
function Dealer({ navigation, route }) {
    ...
    const getBrokers = async () => {
        ...
        await fetch(`http://www.softsauda.com/deal_entry/party_subbroker?masterid=${masterid}`, data)
            ...
            .then(data => {
                ...
                data.results.map(dat => (
                    setBrokerItems(oldArray => [...oldArray, { id: dat.id, name: dat.party_name + "," + dat.cityname }])
    ...
    useEffect(() => {
        if (typeof (buyerId) !== "undefined" && typeof (selllerId) !== "undefined") {
            var count = 0;
            if (contract_sauda_group === undefined || contract_sauda_group === null) {
                        ...
                        <Form style={styles.form}>
                            ...
                            </View>
                            {
                                brokeritems.length == 0 ? (<></>) : (
                                    <View style={styles.column}>
                                        <SelectTwo items={brokeritems} selectedItem={selectedBrokerItems} width={wp('37%')} handleId={handleBrokerId} placeholder="Broker" borderColor="#ccc" defaultValue={broker} />
                                        <DropDownPicker
                                            items={[
                                                { label: 'Buyer', value: 'B' },
                                            ...
                                            dropDownStyle={{ backgroundColor: '#fafafa', width: wp('37%'), marginTop: 0, position: "absolute", zIndex: 20, color: "black" }}
...
/>
                                            ...
                                            <View style={styles.column}>
                                                ...
                                                <TextInput
                                                    keyboardType="numeric"
                                                    style={[styles.input, { backgroundColor: "#D3FD7A", width: wp('37%') ,zIndex:-1}]}
                                                    name="amount"
                                                    placeholder="Amount"
