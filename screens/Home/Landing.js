...
function Landing(props) {
  ...
  useEffect(() => {
    const use = async () => {
      ...
      var divisionNm=await AsyncStorage.getItem("divisionNm")
      props.navigation.setOptions({
        headerTitle: () => {
          return(
            <View style={{diplay:"flex",flexDirection:"row"}}>
            <Text style={{textAlign:"center",fontWeight:"bold",fontSize:wp("5%"),marginTop:5}}>Home  </Text>
            <View style={{diplay:"flex",flexDirection:"column",alignItems:"center",marginLeft:20}}>
            <Text style={{fontWeight:"bold",color:"blue",marginRight:30}} >{divisionNm}                   </Text>
            <Text style={{fontWeight:"bold",color:"blue",marginRight:30}} >{startDate}-{endDate}             </Text>
            </View>
            </View>
          )
        }
