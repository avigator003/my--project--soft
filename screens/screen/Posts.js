...
const[cardHeight,setCardHeight]=useState(hp("58%"))
...
const[loading,setLoading]=useState(true)
 ...
 // fetch party List
    ...
    const _renderSoftSaudaItem = ({item, index}) => {
      ...
      return (
        <View >
          <View style={[styles.card,{height:hp("40%"),backgroundColor:"skyblue",justifyContent:"center",alignItems:"center"}]}>
      ...
      return (
       ...
       <View>
        ...
        <Header  imageUri={`http://www.softsauda.com/+${item.url}`} name={item.partyname?item.partyname:""} address={user && user!=="demo"?item.partyAddress:""} contact={user && user!=="demo"?item.mobile:""} email={user && user!=="demo"?item.email:""}
         firmDescription={item.firmDescription?item.firmDescription:""} 
     />
